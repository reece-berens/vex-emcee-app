...
namespace VEXEmcee.Logic.InternalLogic.MatchList
{
	...
	{
		/// <summary>
		/// Retrieves a list of matches for a specified session, based on the event and division linked to the session.
		/// </summary>
		...
		/// event's statistics are ready, the method retrieves the match list using the appropriate program logic.</remarks>
		/// <param name="request">The request containing the session ID and other parameters required to retrieve the match list.</param>
		/// <returns>A <see cref="GetMatchListResponse"/> object containing the match list, success status, and additional information
		...
		internal static async Task<GetMatchListResponse> GetMatchList(GetMatchListRequest request)
		{
			GetMatchListResponse response = new()
			{
				Success = true,
			...
			//If all stats are ready, find the program used for the event and call the appropriate helper method to get the match list
			...
			else
			{
				Definitions.Event thisEvent = await Accessors.Event.GetEventByID(thisSession.SelectedEventID) ??
					throw new LogicException(1, $"The event linked to the session does not exist. - {request.Session} {thisSession.SelectedEventID}");
				if (thisEvent.StatsReady)
				{
					switch (thisEvent.ProgramID_denorm)
					{
						case 1:
							//V5RC
							await V5RC.GetMatchList(request, response, thisEvent);
							response.ProgramAbbreviation = "V5RC";
							break;
						default:
							response.ErrorMessage = "The selected event's program is not supprted for match list retrieval.";
							response.Matches = [];
							response.Success = false;
							response.StatusCode = System.Net.HttpStatusCode.BadRequest;
							break;
